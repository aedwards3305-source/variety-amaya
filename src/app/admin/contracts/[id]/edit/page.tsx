...
export default function EditContractPage() {
  ...
  const loadContract = useCallback(async () => {
    ...
    try {
      ...
      setForm({
        ...
        totalPrice: contract.totalPrice ? String(contract.totalPrice) : "",
      ...
      setErrors(["Failed to load contract"]);
    } finally {
      setLoading(false);
    }
  }, [id, pwd]);
  ...
  const saveContract = async (asDraft: boolean) => {
    if (asDraft) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <main className="pt-24 pb-16 max-w-3xl mx-auto px-4 lg:px-6">
        <Link href={`/admin/contracts/${id}`} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-[#D4AF37] mb-6 transition-colors">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 mb-8">
            ...
            <textarea
              ...
              rows={4}
              className={`${inputClass} resize-none`}
              placeholder="Any special conditions, notes, or additional terms..."
            />
          </div>

          {/* Actions */}
          <div className="flex items-center justify-between">
            <Link href={`/admin/contracts/${id}`} className="text-gray-400 hover:text-white text-sm transition-colors">
              Cancel
            </Link>
            <div className="flex items-center gap-3">
              {originalStatus === "draft" && (
